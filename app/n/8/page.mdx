...
login, browser launch, and SSO authentication. Each layer is independent — deploy as much or
as little as needed. A local AI assistant layer (covered in [Part 6](/n/9)) builds on top of
this infrastructure.

<Table data={{
  headers: ["Layer", "Tool", "Purpose"],
  rows: [
    ["Power", "BIOS RTC / WoL + Task Scheduler", "Wake at 8am, shut down at 5pm"],
    ["Login", "Sysinternals Autologon", "Unattended Windows login"],
    ["Browser", "Edge + Task Scheduler", "Launch dashboards fullscreen on logon"],
    ["SSO", "Playwright", "Handle identity provider button clicks"],
  ]
}} />
...
- **BIOS/UEFI RTC Wake (Recommended)** — Set a daily wake time in firmware under Power
  Management. Labels vary: "Auto Power On", "Scheduled Boot", "RTC Alarm".
- **Wake-on-LAN (WoL)** — Requires machines left in sleep/hibernate. A server sends a magic
...
<Important>
Use a dedicated service account with read-only dashboard access — not a personal or admin
account.
</Important>
...
require a button click. Those that authenticate silently need no further work. Those that
require interaction are handled by Playwright in Part 4.
...
Handles identity provider login buttons deterministically — no image recognition, no
hardcoded coordinates, no AI. Selectors are DOM-based and fault-tolerant.
