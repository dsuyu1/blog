...
# Why C++ (and not Python/PyPy here)

This problem is tight `O(n log totalSum)` integer DP with heavy looping and random access over big arrays. Even with the right algorithm, Python/PyPy often time out due to interpreter overhead inside those loops.

C++ tends to be the default in contests for this kind of task because:
- the loops compile to optimized machine code
- `vector<int>` / `vector<long long>` are contiguous and cache-friendly
- fast buffered I/O is straightforward

Rust can also be fast enough, but C++ usually has the smoothest contest workflow and the biggest library/pattern ecosystem.
