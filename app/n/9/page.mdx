...
No cloud dependencies. The model reasons over defined tools and invokes them on demand —
...
The LLM does not answer questions directly — it selects a tool and arguments. Python executes
the tool and passes the result back. The model then formulates a spoken response.
...
<Important>
`headless=True` is fine for reads — it does not affect the display windows already open.
